...
class SymmetricDilatedConv2d(nn.Module):
    """Symmetric temporal convolution + spatial 2D convolution."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_xy = x.permute(0, 2, 1, 3, 4).reshape(B * T, C, X, Y)
        out_xy = self.conv_xy(x_xy)
        C_out = out_xy.shape[1]
        out_xy = out_xy.reshape(B, T, C_out, X, Y).permute(0, 2, 1, 3, 4)

        x_t = x.permute(0, 3, 4, 1, 2).reshape(B * X * Y, C, T)
        out_t = self.conv_t(x_t)
        out_t = out_t.reshape(B, X, Y, C_out, T).permute(0, 3, 4, 1, 2)
...
class MRScaleBlock2D(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, dilation: int = 1, spatial_padding_mode: str = "circular"):
        ...
        self.conv2 = SymmetricDilatedConv2d(
            out_channels,
            out_channels,
            kernel_size=3,
            dilation=dilation,
        ...
        self.skip = nn.Conv3d(int(in_channels), int(out_channels), kernel_size=1) if int(in_channels) != int(out_channels) else nn.Identity()
...
class MultiResolutionEncoder2D(nn.Module):
    """2D Multi-Resolution Spatiotemporal Feature Extractor.
    ...
    def __init__(
        ...
        self.scale_adapters = nn.ModuleList(
            [nn.Conv3d(out_dim, out_dim, kernel_size=1) for out_dim in self.scale_dims]
        )
    ...
    def _feature_concat(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for l, (block, adapter) in enumerate(zip(self.scale_blocks, self.scale_adapters)):
            if l > 0:
                sf = 2 ** l
                x_down = F.avg_pool3d(x_conv, kernel_size=(1, sf, sf), stride=(1, sf, sf))
            else:
                x_down = x_conv

            feat = block(x_down)
            if l > 0:
                feat = F.interpolate(feat, size=(T, X, Y), mode="trilinear", align_corners=False)
            feats.append(adapter(feat))

        return torch.cat(feats, dim=1).permute(0, 2, 3, 4, 1)
...
if __name__ == "__main__":
    model = MultiResolutionEncoder2D(in_channels=2, d_h=128, n_scales=3)
    x = torch.randn(2, 20, 32, 32, 2)
    out = model(x)
    print(f"MultiResolutionEncoder2D: input {tuple(x.shape)} -> output {tuple(out.shape)}")
